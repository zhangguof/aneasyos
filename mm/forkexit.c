...
//	send_recv(BOTH, TASK_FS, &msg2fs);
	...
	if (proc_table[parent_pid].p_flags & WAITING) { /* parent is waiting */
		...
		printf("{MM} ((--do_exit():1: proc_table[parent_pid].p_flags: 0x%x--))\n",
		...
		printf("{MM} ((--do_exit():: %s (%d) is not WAITING, %s (%d) will be HANGING--))\n",
		       proc_table[parent_pid].p_name, parent_pid,
		       p->p_name, pid);
